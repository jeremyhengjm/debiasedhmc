...
# grid_stepsize <- seq(0.05, 0.19, by = 0.02)
...
for (igrid in 1:ngrid_stepsize){
  filename <- paste("inst/coxprocess/output.rm.hmc.tuning", igrid, ".RData", sep = "")
  load(file = filename)
  # results.df <- rbind(results.df, data.frame(stepsize = rep(grid_stepsize[igrid], ngrid_nsteps-1),
                                             # nsteps = grid_nsteps[2:4],
                                             # var = variance[2:4] / dimension * grid_nsteps[2:4] ,
                                             # acceptprob = acceptprob[2:4]))
...
save(grid_stepsize, ngrid_stepsize, grid_nsteps, ngrid_nsteps,
     results.df, file = "inst/coxprocess/output.rm.hmc.tuning.RData")

# Optimal marginal RM-HMC kernel
load("inst/coxprocess/output.rm.hmc.tuning.RData")
index_min <- na.omit((results.df$var == min(results.df$var, na.rm = TRUE)))
cat("Optimal marginal RM-HMC kernel")
...
g
ggsave(filename = "inst/coxprocess/coxprocess.rm.hmc.tuning.pdf", plot = g, width = 9, height = 6)
