...
for (igrid in 1:ngrid_stepsize){
  filename <- paste("inst/coxprocess/output/output.rm.hmc.tuning", igrid, ".RData", sep = "")
  load(file = filename)
...
save(grid_stepsize, ngrid_stepsize, grid_nsteps, ngrid_nsteps,
     results.df, file = "inst/coxprocess/output/output.rm.hmc.tuning.RData")

# Optimal marginal RM-HMC kernel
load("inst/coxprocess/output/output.rm.hmc.tuning.RData")
...
g
ggsave(filename = "inst/coxprocess/plots/coxprocess.rm.hmc.tuning.pdf", plot = g, width = 9, height = 6)
