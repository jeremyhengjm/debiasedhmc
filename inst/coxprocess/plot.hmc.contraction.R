...
for (igrid in 1:ngrid_stepsize){
  filename <- paste("inst/coxprocess/output/output.hmc.contraction", igrid, ".RData", sep = "")
  load(file = filename)
...
save(nreps, grid_stepsize, ngrid_stepsize, grid_nsteps, ngrid_nsteps,
     results.df, max_results.df, file = "inst/coxprocess/output/output.hmc.contraction.RData")

# distance after 1000 iterations (no colour)
load(file = "inst/coxprocess/output/output.hmc.contraction.RData")
...
g
# ggsave(filename = "inst/coxprocess/plots/coxprocess.hmc.contraction.pdf", plot = g, width = 9, height = 6)
...
contraction.df
dim(contraction.df)
save(contraction.df, file = "inst/coxprocess/hmc.contraction.parameters.RData")
